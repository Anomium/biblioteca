...
class PrestamoTests {
    ...
    @Test
    public void prestamoConIdentificacionUsuarioNoPermitidoDeberiaRetornarExcepcion() throws Exception {

        mvc.perform(MockMvcRequestBuilders
                        .post("/prestamo")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SolicitudPrestarLibroTest("EQWQW8545", "0111111111", USUARIO_INVITADO))))
                .andDo(print())
                .andExpect(status().is4xxClientError())
                .andExpect(jsonPath("$.mensaje", is("Cedula no valida. (Permitido: Numerico y que comiencen por numero diferente de cero(0)).")));
    }

    @Test
    public void prestamoConLetrasEnIdentificacionUsuarioNoPermitidoDeberiaRetornarExcepcion() throws Exception {

        mvc.perform(MockMvcRequestBuilders
                        .post("/prestamo")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SolicitudPrestarLibroTest("EQWQW8545", "ABC1111111", USUARIO_INVITADO))))
                .andDo(print())
                .andExpect(status().is4xxClientError())
                .andExpect(jsonPath("$.mensaje", is("Cedula no valida. (Permitido: Numerico y que comiencen por numero diferente de cero(0)).")));
    }
